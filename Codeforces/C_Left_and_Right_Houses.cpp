...
cin.tie(NULL);
    ...
    while(t--){
        ...
        int ans1=-1,ans2=-1;
        double val1=1e9,val2=1e9;;
        for(int i=n/2-1;i>=0;i--){
            if((pz[i]>=(i+2)/2 && so[i]>=(n-i)/2 )){
                val1=(double)(n/2)-i;
                ans1=i;
                break;
            }
        ...
        if(ans1==-1 && ans2!=-1){
            ...
            if(so[0]>=(n+1)/2)cout<<0<<endl;
            else if(pz[n-1]>=(n+1)/2)cout<<n-1<<endl;
            else cout<<0<<endl;
        }
        else{
            if(val1<=val2)cout<<ans1+1<<endl;
