...
int SumOfKsubArray(vi &arr , int n , int k)
{
    int sum = 1e9;  // Initialize result
    ...
    for (  ; i < n; i++ )
    {
        ...
        sum =min(sum, -arr[S.front()] + arr[G.front()]);
...
cin.tie(NULL);
    ...
    while(t--){
          ...
          int ans=n+1;
          vi vals(n);
          for(int i=0;i<n;i++)vals[i]=fre[i+1];
          cout<<SumOfKsubArray(vals,n,k)<<endl;
