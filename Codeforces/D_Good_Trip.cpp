...
cin.tie(NULL);
    ...
    while(t--){
        ...
        ll invdenom=power(denom, mod-2, mod);
        ll ic=(sum*k)%mod;
        ic*=(invdenom)%mod;
        ll incval=0;
        for(int i=1;i<=k;i++){
            ll contvalue=((i*(i-1))/2)%mod;
            ll ff=nCrModPFermat(k,i,mod);
            ll sf=power(invdenom,i,mod);
            ll pr=((denom-1)*invdenom)%mod;
            ll tf=power(pr,k-i,mod);
            ll prob=(ff*sf)%mod;
            prob=(prob*tf)%mod;
            prob=(prob*contvalue)%mod;
            incval=(incval+prob)%mod;
        }
        cout<<(ic+(incval*m)%mod)%mod<<endl;
    }
    return 0;
}