...
ll nCrMoszFermat(ll n,ll r, ll p)
{
if (n < r)
return 0;
if (r == 0)
    ...
    while(t--){
        ...
        for(int i=1;i<=n;i++){
            ...
            if(t==1){
               ...
               sz[i] = ((v + 1) > 2e18 / sz[i - 1]) ? (ll)2e18 : sz[i - 1] * (v + 1);
                lst[i]=lst[i-1];
                if(valid)poss.push_back(i);
            }
            if(sz[i]==2e18)valid=false;
        }
        while(q--){
            int x;cin>>x;
            if(!poss.empty()){
           ...
           int idx=lower_bound(sz.begin()+1,sz.end(),x)-sz.begin();
            cout<<lst[idx]<<" ";
        }
        cout<<endl;
