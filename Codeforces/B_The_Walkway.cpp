...
cin.tie(NULL);
    ...
    while(t--){
        ...
        int wr=0;
        pos.insert(pos.begin(),1-d);
        pos.push_back(n+1);
        for(int i=1;i<pos.size();i++){
            wr+=(pos[i]-pos[i-1]-1)/d;
        }
        wr+=(int)(pos.size()-2);
        //eating at all workers places
        int cnt=0;
        ll ans=inf;
        for(int i=1;i<=m;i++){
            int val1=pos[i]-pos[i-1]-1;
            int val2=pos[i+1]-pos[i]-1;
            int val3=pos[i+1]-pos[i-1]-1;
            int neweat=val3/d-(val1/d+val2/d);
            if(neweat<ans){
                ans=neweat;
                cnt=0;
            }
