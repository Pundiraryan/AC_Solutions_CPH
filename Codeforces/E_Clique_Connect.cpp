...
cin.tie(NULL);
    ...
    while(t--){
          ...
          cin>>n>>m;
          DSU d(n);
          vector<pair<int,vi>>fre;
          for(int i=0;i<m;i++){
            int k,c;cin>>k>>c;
            vi nums(k);
            cin>>nums;
            fre.push_back({c,nums});
            for(int i=1;i<k;i++)d.join(nums[i-1],nums[i]);
          }
          bool anse=false;
          for(int i=1;i<=n;i++){
            if(d.sz[i]==n){
          ...
          if(!anse){
            ...
            for(int i=0;i<fre.size();i++){
                // cout<<fre[i].first<<" --- \n";
                vi vals=fre[i].second;
                for(int j=1;j<vals.size();j++){
                    if(d1.find(vals[j])==d1.find(vals[j-1]))continue;
                    d1.join(vals[j],vals[j-1]);
                    ans+=fre[i].first;
                    // cout<<ans<<" ++  \n";
                }
            }
