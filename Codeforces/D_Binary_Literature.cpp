...
cin.tie(NULL);
    ...
    while(t--){
        ...
        int cz=ans.size();
        if(i>=cz-n && i!=2*n){
            for(int j=i;j<2*n;j++)ans.push_back(a[j]);
        }
        else if(j>=cz-n && j!=2*n){
            for(int i=j;i<2*n;i++)ans.push_back(b[i]);
        }
        else if(k>=cz-n && k!=2*n){
            for(int l=k;l<2*n;l++)ans.push_back(c[l]);
        }
        cout<<ans<<endl;
    }
    return 0;
