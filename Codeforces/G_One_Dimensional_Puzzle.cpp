...
cin.tie(NULL);
    ...
    while(t--){
        ...
        int c1,c2,c3,c4;
        cin>>c1>>c2>>c3>>c4;
        int dp[c1+c2+c3+c4+1][4];
        mem(dp,0);
        int val=c1+c2+c3+c4;
        dp[0][0]=1;dp[0][1]=1;dp[0][2]=1;dp[0][3]=1;
        for(int i=0;i<(c1+c2+c3+c4);i++){
            if(c1>0){
                dp[i+1][1]+=(dp[i][0])%mod1;
                dp[i+1][2]+=(dp[i][0])%mod1;
                c1--;
            }
            if(c2>0){
                 dp[i+1][0]+=(dp[i][1])%mod1;
                 dp[i+1][3]+=(dp[i][1])%mod1;
                 c2--;
            }
            if(c3>0){
                dp[i+1][1]+=(dp[i][2])%mod1;
                dp[i+1][2]+=(dp[i][2])%mod1;
                c3--;
            }
            if(c4>0){
                 dp[i+1][0]+=(dp[i][3])%mod1;
                 c4--;
            }
        }  
        int ans=0;
        for(int i=0;i<4;i++){
            ans=(ans+dp[val][i])%mod1;
        }
        cout<<ans<<endl;
    }
    return 0;
