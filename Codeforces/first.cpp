...
int mini(int v,int tl,int tr,int l,int r,vi &arr){
    if(l>r)return 1e9;
    if(l==tl&&r==tr){
        return segt[v];
    }
    int tm=(tl+tr)/2;
    return min(mini(2*v,tl,tm,l,min(r,tm),arr),mini(2*v+1,tm+1,tr,max(tm+1,l),r,arr));
}
...
cin.tie(NULL);
    ...
    // cin>>t;
    while(t--){
          vector<int>v={2,22,6,1,5,7};
          int n=v.size();
          segt.resize(4*n +1);
          build(1,0,n-1,v);
          update(1,0,n-1,n-1,2,v);
          cout<<mini(1,0,n-1,4,n-1,v)<<endl;
