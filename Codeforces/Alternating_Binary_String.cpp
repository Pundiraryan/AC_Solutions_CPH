...
ll f(int i,int ns,vector<int>&nums,vector<vector<long long>>&dp){
    int n=nums.size();
    if(i==n)return 0;
    if(dp[i][ns]!=-1)return dp[i][ns];
    ll ans=-1e14;
    if(ns==0){
         ans=max(ans,nums[i]+max(f(i+1,0,nums,dp),f(i+1,1,nums,dp)));
    }else{
       ans=max(ans,f(i+1,0,nums,dp)-nums[i]);
    }
    return dp[i][ns]= ans;
}
signed main(){
