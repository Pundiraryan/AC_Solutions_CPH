...
cin.tie(NULL);
    ...
    while(t--){
        ...
        vpi sa,sb,sc;
        sa.push_back(a[n-1]);sa.push_back(a[n-2]);sa.push_back(a[n-3]);
        sb.push_back(b[n-1]);sb.push_back(b[n-2]);sb.push_back(b[n-3]);
        sc.push_back(c[n-1]);sc.push_back(c[n-2]);sc.push_back(c[n-3]);
        int ans=0;
        si st;
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                for(int k=0;k<3;k++){
                    if(sa[i].second!=sb[j].second && sb[j].second!=sc[k].second && sc[k].second!=sa[i].second){
                        ans=max(ans,sa[i].first+sb[j].first+sc[k].first);
                    }
                }
            }
        }
