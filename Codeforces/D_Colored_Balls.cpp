...
ll f(int i,int li,vi&nums){
    int n=nums.size();
    if(i>=n)return li;
    // if(dp[i][li]!=-1)return dp[i][li];
    ll take=(ll)f(i+1,nums[i]-li,nums)+li;
    ll nottake=f(i+1,li,nums);
    ll res=0;
    res=(res+take)%mod1;
    res=(res+nottake)%mod1;
    return res;
}
