...
cin.tie(NULL);
    ...
    while(t--){
        ...
        for(int i=0;i<v.size();i++){
            ...
            sort(all(cstr));
           if(narr.empty()){
            if(cstr==v[i] && stoi(v[i])>=10){
                for(auto &x:cstr)narr.push_back(x-'0');
            }else{

                narr.push_back(stoi(v[i]));
            }
           }else{
            if(stoi(v[i])<narr.back()){
                anse=false;break;
            }
            if(cstr==v[i] && narr.back()<=v[i][0]-'0' &&  stoi(v[i])>=10){
                for(auto &x:cstr)narr.push_back(x-'0');
            }else{
       ...
       if(!is_sorted(all(narr)))anse=false;
       if(anse)cout<<"YES\n";
       else cout<<"NO\n";
    }
    return 0;
