...
cin.tie(NULL);
    ...
    while(t--){
        ...
        vector<vector<int>>ps1(317,vector<int>(n+317));
        vector<vector<int>>ps2(317,vector<int>(n+317));
        for (int d = 1; d < 316; d++) {
            for (int i = 0; i < n; i++) {
                ps1[d][i+d] = ps1[d][i]+ nums[i]*(i/d+1);
                ps2[d][i+d] = ps2[d][i]+nums[i];
            }
        }
    while(q--){
        int s,d,k;
        ...
        if (d>=316){
            ...
            int r = s+d*k;
            cout<<ps1[d][r] - ps1[d][s] - (s/d)*(ps2[d][r]-ps2[d][s])<<" ";
        }
    }
    cout<<endl; 
    }
