...
cin.tie(NULL);
    ...
    while(t--){
        ...
        DSU d(n);
        for(int i=0;i<n;i++){
            int val=nnums[i].first;
            int idx=nnums[i].second;
            if(val>=p)break;
            int j1=idx-1,j2=idx+1;
            // cout<<j1<<" -- "<<j2<<endl;
            while(true){
                if(j1<0)break;
                if(nums[j1]<val)break;
                if(nums[j1]%val!=0)break;
                if(d.find(j1)==d.find(idx))break;
                d.join(idx,j1);
                ans+=val;
                j1--;
            }
             while(true){
                if(j2>=n)break;
                if(nums[j2]<val)break;
                if(nums[j2]%val!=0)break;
                if(d.find(j2)==d.find(idx))break;
                d.join(idx,j2);
                ans+=val;
                j2++;
            }
