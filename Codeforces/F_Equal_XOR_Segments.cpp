...
cin.tie(NULL);
    ...
    while(t--){
        ...
        vi prexor(n+1,0);
        map<int,vi>fre;
        for(int i=1;i<=n;i++)prexor[i]=prexor[i-1]^nums[i-1];
        for(int i=0;i<=n;i++){
            fre[prexor[i]].push_back(i);
        }
        while(q--){
            int l,r;cin>>l>>r;
            if(prexor[r]==prexor[l-1]){
                ...
                auto lpos=lower_bound(fre[prexor[l-1]].begin(),fre[prexor[l-1]].end(),r);
                --lpos;
                int lidx=*lpos;
                auto rpos=lower_bound(fre[prexor[r]].begin(),fre[prexor[r]].end(),l);
                int ridx=*rpos;
                if(ridx<lidx){
                    // pl-1 pr pl-1 pr 
                    cout<<"YES\n";
