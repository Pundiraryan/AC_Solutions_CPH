...
private:
    ...
    int _query(int l, int r, int v, int tl, int tr) {
        if(lazy[v] != inf) {
            ...
            if(tl != tr) {
                lazy[2*v] = min(lazy[v], lazy[2*v]);
                lazy[2*v + 1] = min(lazy[v], lazy[2*v + 1]);
            }
            lazy[v] = inf;
        }
        ...
        int m = (tl + tr) / 2;
        return min(_query(l, r, 2*v, tl, m), _query(l, r, 2*v + 1, m + 1, tr));
    }
    ...
    void _update(int l, int r, int val, int v, int tl, int tr) {
        ...
        if(l <= tl && tr <= r) {
            ...
            if(tl != tr) {
                lazy[2*v] = min(val, lazy[2*v]);
                lazy[2*v + 1] = min(val, lazy[2*v + 1]);
            }
            return;
        }
 
        int m = (tl + tr) / 2;
        _update(l, r, val, 2*v, tl, m);
        _update(l, r, val, 2*v + 1, m + 1, tr);
...
public:
    ...
    void update(int l, int r, int val) {
        return _update(l, r, val, 1, 0, n - 1);
    }
};
...
cin.tie(NULL);
    ...
    while(t--){
        ...
        for(int i=0;i<n;i++){
            MinSegTree tree(m);
        tree.update(0, 0, grid[i][0] + 1);
        for(int j = 1; j < m - 1; j++) {
            int res = tree.query(max(j - d - 1LL, 0LL), j) + grid[i][j] + 1;
            tree.update(j, j, res);
        }
 
        // now we get minimum with last res
        dpf[i] = tree.query(max(m - 1 - d - 1, 0LL), m - 2) + grid[i][m - 1] + 1;
        }
        // cout<<dpf<<endl;
        ll cs=0;
        for(ll i=0;i<k;i++){
