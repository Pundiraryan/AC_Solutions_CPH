...
cin.tie(NULL);
    ...
    while(t--){
          string a,b;cin>>a>>b;
          map<char,set<int>>fre;
          int n=a.length();
          for(int i=0;i<n;i++){
            fre[a[i]].insert(i);
          }
          char ch1=(char)tolower(b[0]),ch2=(char)tolower(b[1]),ch3=(char)tolower(b[2]);
        //   cout<<ch1<<" "<<ch2<<" "<<ch3<<endl;
          if(fre[ch1].empty()){
            cout<<"No\n";
            continue;
          }
          int idx1=*fre[ch1].begin();
          if(fre[ch2].empty()){
            cout<<"No\n";
            continue;
          }
          auto  idx2=fre[ch2].lower_bound(idx1+1);
           if(idx2==fre[ch2].end()){
            cout<<"No\n";
            continue;
           }   
           ...
           auto idx3=fre[ch3].lower_bound(sidx+1);
           if(idx3==fre[ch3].end()){
            cout<<"No\n";
            continue;
           }
