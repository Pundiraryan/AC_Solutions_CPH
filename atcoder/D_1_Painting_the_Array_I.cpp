...
// int f(int i,int fv,int sv,vi&nums){
//     int n=nums.size();
//     if(i==n)return 0;
//     if(dp.count(i) && dp[i].count(fv) && dp[i][fv].count(sv))return dp[i][fv][sv];
//     int case1=f(i+1,nums[i],sv,nums)+(nums[i]!=fv);
//     int case2=f(i+1,fv,nums[i],nums)+(nums[i]!=sv);
//     return dp[i][fv][sv]=max(case1,case2);
// }
signed main(){
...
cin.tie(NULL);
    ...
    while(t--){
        ...
        cin>>n;
        vi nums(n);cin>>nums;
        // cout<<f(0,-1,-1,nums)<<endl;
        int b1=0,b2=0;
        int ans=n;
        for(int i=0;i<n;i++){
            if(nums[i]!=b1 && nums[i]!=b2){
                // 0 1 -- 2 0
                if(i!=n-1){
                    if(nums[i+1]==b1){
                        b1=nums[i];
