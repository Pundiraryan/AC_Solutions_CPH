...
cin.tie(NULL);
    ...
    while(t--){
        ...
        string s;cin>>s;
        int ans=2e9;
        // 0000111000 for every string containing k distant chars
        int to=0;
        for(auto &xx:s)to+=(xx=='1');
        auto f=[&](string &str)->int{
            int n=str.length();
            vector<vi>dp(n+1,vi(3,0));
            for(int i=0;i<n;i++){
                dp[i+1][0]=dp[i][0]+(str[i]=='1');
                dp[i+1][1]=min(dp[i][1],dp[i][0])+(str[i]=='0');
                dp[i+1][2]=min({dp[i][0],dp[i][1],dp[i][2]})+(str[i]=='1');
            }
