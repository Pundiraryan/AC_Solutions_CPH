...
ll invs[maxn+1];
void inverses(){
    invs[maxn] = power(factorial[maxn],mod-2,mod);
    for(int i=maxn-1;i>=0;i--) invs[i] = invs[i+1]*(i+1)%mod;
}
 
ll ncr(int n,int r){
    return (((factorial[n]*invs[n-r])%mod)*invs[r])%mod;
}
int logb2(ll x){ return __builtin_clzll(1ll) - __builtin_clzll(x); }
ll dp[200005][2];
ll f(ll bit,ll tight,ll n,ll k){
    if(bit==k)return 1;
    if(dp[bit][tight]!=-1)return dp[bit][tight];
    ll ans=0;
    if(!tight){
        ans=(ll)(power(2,n,mod)*f(bit+1,0,n,k)%mod);
    }
    else{
        // 00 or 10
        if(n%2==0){
            ans=(ans+f(bit+1,0,n,k))%mod;
            ll val=power(2,n-1,mod)-1;
            val=(val*f(bit+1,1,n,k))%mod;
            ans=(ans+val)%mod;
        }else{
            ans=(ans+f(bit+1,1,n,k))%mod;
            ll val=power(2,n-1,mod);
            val=(val*f(bit+1,1,n,k))%mod;
            ans=(ans+val)%mod;
        }
    }
    return dp[bit][tight]= ans;
}
signed main(){
