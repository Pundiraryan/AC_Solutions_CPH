...
cin.tie(NULL);
    ...
    while(t--){
        ...
        //   cout<<endl
          ...
          sort(all(v));
          vector<int>suffmin(n,2e9);
          suffmin[n-1]=v[n-1].second;
          for(int i=n-2;i>=0;i--){
            suffmin[i]=min(suffmin[i+1],v[i].second);
          }
          vi inans(n,1);
        //   for(int i=0;i<n;i++)er.insert(i);a
          for(int i=0;i<n-1;i++){
            pi ts={v[i].first+1,-1};
            auto idx=lower_bound(all(v),ts)-v.begin();
            if(idx==n)continue;
            if(suffmin[idx]<v[i].second){
                // cout<<fre[v[i]]<<" -- \n";
                inans[fre[v[i]]]=0;
            }
          ...
          vi er;
          for(int i=0;i<n;i++)if(inans[i])er.push_back(i);
          cout<<er.size()<<endl;
          for(auto &x:er)cout<<x+1<<" ";
          cout<<endl;
    }
    return 0;
}