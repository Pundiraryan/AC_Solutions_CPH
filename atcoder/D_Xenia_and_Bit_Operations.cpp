...
vi segt;
int build(int v,int tl,int tr,vi&arr){
    if(tl==tr){
        segt[v]=arr[tl];
        return 0;
    }
    int tm=(tl+tr)/2;
    int v1=build(2*v,tl,tm,arr);
    int v2=build(2*v+1,tm+1,tr,arr);
    if(v1==v2){
        if(!v1){
...
int update(int v,int tl,int tr,int pos,int x,vi&arr){
    if(tl==tr){
        segt[v]=x;
        return 0;
    }
    ...
    int v1;
    if(pos<=tm){
        v1=update(2*v,tl,tm,pos,x,arr);
    }
    else v1=update(2*v+1,tm+1,tr,pos,x,arr);
    if(v1){
        segt[v]=segt[2*v]^segt[2*v+1];
    }else{
        segt[v]=segt[2*v]|segt[2*v+1];
...
cin.tie(NULL);
    ...
    while(t--){
          ...
          int sz=(1<<n);
          segt.resize(4*sz+5);
          build(1,0,(1<<n)-1,nums);
          for(int i=0;i<m;i++){
            int a,b;
            cin>>a>>b;
            --a;
            update(1,0,(1<<n)-1,a,b,nums);
            cout<<segt[1]<<endl;
          }
    }
    return 0;
