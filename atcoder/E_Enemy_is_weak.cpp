...
public:
    ...
    int query1 (int x, int y, int k) {
        return querylower (x, y, k, 0, n-1, 0);
    }
    int  query2(int x,int y,int k){
        return queryhigher (x, y, k, 0, n-1, 0);
    }
...
private:
    ...
    int querylower (int x, int y, int k, int l, int r, int i) {
        if (r < x || l > y) return 0;
        if (l >= x && r <= y) {
            int ind = lower_bound(tree[i].begin(), tree[i].end(), k) - tree[i].begin();
            return (ind);
        }
 
        int m = (l+r) >> 1;
        return (
            querylower (x, y, k, l, m, i*2+1) + 
            querylower (x, y, k, m+1, r, i*2+2)
        );
    }
    int queryhigher (int x, int y, int k, int l, int r, int i) {
        if (r < x || l > y) return 0;
        if (l >= x && r <= y) {
        ...
        int m = (l+r) >> 1;
        return (
            queryhigher (x, y, k, l, m, i*2+1) + 
            queryhigher (x, y, k, m+1, r, i*2+2)
        );
    }
...
cin.tie(NULL);
    ...
    while(t--){
          ...
          ll ans=0;
          for(int j=0;j<n;j++){
            ans=(ans+(ll)(st.query2(0,j-1,nums[j])*st.query1(j+1,n-1,nums[j])));
          }
          cout<<ans<<endl;
    }
    return 0;
