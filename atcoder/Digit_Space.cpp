...
vs dx,dy;
void genp(int i,string &str,vs&v)
{
    int n=str.length();
    if (i==n-1)
    {
        if(str.front()!='0')v.push_back(str);
        return;
    }
    for (int j=i;j<n;j++)
    {
...
cin.tie(NULL);
    ...
    while(t--){
          ...
          genp(0,fn,dx);
          genp(0,sn,dy);
          unordered_set<int> pf1,pf2;
          for(auto &x:dx){
            int val=stoi(x);
            // cout<<"for x --"<<val<<endl; 
            vi facs=giveallfacs(val);
            // for(auto &x:facs)cout<<x<<endl;
            for(auto &x:facs)pf1.insert(x);
          }
          ...
          for(auto &y:dy){
            int val=stoi(y);
            // cout<<"for y --"<<val<<endl; 
            vi facs1=giveallfacs(val);
            // for(auto &x:)
            for(auto &x:facs1){
                if(pf1.find(x)!=pf1.end()){
                    ans=max(ans,x);
                }
            }
          }
