...
cin.tie(NULL);
    ...
    while(t--){
        ...
        vi nums(n);cin>>nums;
        vl ans(n);
        multiset<ll>bada,chota;
        ll bs=0,cs=0;
        for(int i=0;i<n;i++){
            chota.insert(nums[i]);
            cs+=nums[i];
            while(chota.size()>=bada.size()){
                int cmb=*chota.rbegin();
                cs-=cmb;
                bs+=cmb;
                chota.erase(chota.find(cmb));
                bada.insert(cmb);
            }
            while(true){
                if(chota.empty() || bada.empty())break;
                if(*chota.rbegin()<=*bada.begin())break;;
                int cmb=*chota.rbegin();
                int bmc=*bada.begin();
                cs+=(bmc-cmb);
                bs+=(cmb-bmc);
                chota.erase(chota.find(cmb));
                bada.erase(bada.find(bmc));
                chota.insert(bmc);
                bada.insert(cmb);
            }
            ans[i]=bs-cs;
