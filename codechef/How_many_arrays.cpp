...
cin.tie(NULL);
    ...
    while(t--){
        int n,x;
        cin>>n>>x;
        umpl facs=factorise(x);
        ll val=1;
        // for(auto &x:facs){
        //     val=(val*power(x.first,x.second,mod1))%mod1;
        // }
        // assert(val==x);
        int ans=0;
        for(int len=1;len<=n;len++){
            ll arrs=1;
            for(auto &x:facs){
                ll allcases=power(x.second+1,len,mod1);
                ll nmp=power(x.second,len,mod1);
                allcases=(allcases-nmp+mod1)%mod1;
                arrs=(arrs*allcases)%mod1;
            }
            ans=(ans+arrs)%mod1;
        }
        cout<<ans%mod1<<endl;
