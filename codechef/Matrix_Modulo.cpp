...
{
factorial[0] = 1;
for (ll i = 1; i < 125500; i++) factorial[i] = (factorial[i - 1] * i) % mod; 
}
ll power(ll x, ll y)
{
ll res = 1;
...
cin.tie(NULL);
    ...
    while(t--){
        ...
        for(ll i=0;i<n;i++){
            for(ll j=0;j<n;j++){
                cin>>grid[i][j];
                if(i!=j && (grid[i][j]>sv && grid[i][j]<=mn)){
                    anse=false;
                }
                else if(i==j && grid[i][j]==0)nfd++;
            }
        }  
        if(!anse){
            cout<<0<<endl;
